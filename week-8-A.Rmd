...
q1 <- stan_glm(att_chg ~ treatment - 1,
         data = week_8,
         refresh = 0)
```
...
q1 %>%
  ...
  ggplot(aes(x = ATE, y = after_stat(count/sum(count)))) +
  geom_histogram(alpha = 0.75, bins = 100, fill = "dodgerblue") +
scale_y_continuous(labels=scales::percent_format()) +
theme_classic() +
  labs(x = "Shifts in Attitude",
       y = "Probability",
